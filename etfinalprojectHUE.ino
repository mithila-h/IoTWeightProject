...
const char* bridgeIp = "192.168.1.2";  
const int bridgePort = 80;
const char* apiKey = "2a4cfb8a9e0123456789abcde0123456789abcde";
const int lightId = 1;

// WiFi credentials
const char* ssid = "utexas-iot";
const char* password = "84127065467604067091";
WiFiClient client;
...
void changeColor(status) {
  // Define the color parameters (you can adjust these values)
  if (status = "good"){
    int hue = 46920;    // blue
  } else {
    int hue = 15000;    //orange
  }
  
  int saturation = 254;  // Saturation value (0 to 254)
  int brightness = 254;  // Brightness value (0 to 254)
  ...
  // Make an HTTP PUT request to change the light color
  String url = "/api/" + String(apiKey) + "/lights/" + String(lightId) + "/state";
  if (client.connect(bridgeIp, bridgePort)) {
    ...
    Serial.println("Failed to connect to the Hue Bridge");
  }

  // Read and print the HTTP response (optional)
